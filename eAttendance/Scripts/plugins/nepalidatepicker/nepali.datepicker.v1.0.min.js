...
function setSelectedDay(value) {
    ...
    $('#' + ndp_target_id).val(value).change();
    hideCalendarBox();
    //$('#' + ndp_target_id).closest('input[type=text]').focus();
}
function showCalendar(val) {
    $("#ndp-nepali-box table").find("tr:gt(0)").remove(); if (val == "") { $('#ndp-nepali-box table').append(getDateTable('')); } else { $('#ndp-nepali-box table').append(getDateTable(val)); }
...
function getMonthParameters(date) {
    var currentNepDateParts = date.split('-'); var nepaliYear = currentNepDateParts[0]; var nepaliMonth = currentNepDateParts[1]; $('#currentMonth').html(getNepaliMonth(nepaliMonth - 1) + ' ' + englishNo2Nep(nepaliYear)); nYear = pYear = nepaliYear; nMonth = (parseInt(nepaliMonth, 10) + 1); if (nMonth > 12) { nYear++; nMonth = '01'; }
...
function getDateRows(startDayCode, totalDays, currentYear, currentMonth, currentDay) {
    var nepaliDateNow = getNepaliDate(); var currentNepDateParts = nepaliDateNow.split('-'); var nepaliYear = currentNepDateParts[0]; var nepaliMonth = get2DigitNo(currentNepDateParts[1]); var nepaliDay = get2DigitNo(currentNepDateParts[2]); var returnString = ""; for (var i = 0; i < startDayCode + totalDays; i++) {
        if ((i % 7) == 0) returnString += "<tr>"; var nepDateVal = (i - startDayCode + 1); var loopDateValue = currentYear.toString() + '-' + get2DigitNo(currentMonth) + '-' + get2DigitNo(nepDateVal); var className = ""; if (get2DigitNo(currentMonth) == nepaliMonth && nepaliDay == get2DigitNo(nepDateVal)) { className = "ndp-selected"; } else if (nepDateVal == currentDay) { className = "ndp-current"; } else { className = "ndp-date"; }
...
function englishNo2Nep(no) {
    no = no.toString(); var nepNo = ''; for (var i = 0; i < no.length; i++) { nepNo += convertNos(no[i]); }
    return nepNo;
}
...
function count_bs_days(start_date, end_date) {
    var x = start_date.split("/"); var y = end_date.split("/"); var start_year = Number(x[2]); var start_month = Number(x[0]); var start_days = Number(x[1]); var end_year = Number(y[2]); var end_month = Number(y[0]); var end_days = Number(y[1]); var days = 0; var i = 0; for (i = start_year; i <= end_year; i++)
...
function add_bs_days(bs_date, num_days) {
    var x = bs_date.split("/"); var bs_year = Number(x[2]); var bs_month = Number(x[0]); var bs_days = Number(x[1]); bs_days += num_days; while (bs_days > this.bs[bs_year][bs_month - 1]) {
